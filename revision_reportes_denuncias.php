...
$query = "SELECT COUNT(DISTINCT propuestas_noticias_id) AS total FROM reportes WHERE estado = 'pendiente'";
$result = $conexion->query($query);
if ($result && $row = $result->fetch_assoc()) {
    $reportes_pendientes = $row['total'];
}

$stmt = $conexion->prepare("SELECT * FROM reportesdenuncias WHERE estado = 'pendiente' GROUP BY propuestas_denuncias_id");
$stmt->execute();
$reportes = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
$stmtRepetidos = $conexion->prepare("SELECT propuestas_denuncias_id, COUNT(*) as cantidad FROM reportesdenuncias WHERE estado = 'pendiente' GROUP BY propuestas_denuncias_id");
...
foreach ($resultadoRepetidos as $item) {
    $conteoReportes[$item['propuestas_denuncias_id']] = $item['cantidad'];
}
?>
        ...
        <div class="contenedor">
            ...
            <div class="tooltip" style="margin-left: -60px;">
                <a href="revision_reportes.php">
                    <img src="imagenes/izquierda.png" alt="Reportar" style="width: 50px; height: 50px; cursor: pointer;">
                    <?php if ($reportes_pendientes > 0): ?>
                        <span class="notificacion"><?= $reportes_pendientes ?></span>
                    <?php endif; ?>
                </a>
                <div class="tooltip-text">Reportes Noticia</div>
            </div>
            <h1 class="titulo-session">Revisión de Reportes Pendientes de Denuncias</h1>
        </div>
            ...
            <?php else: ?>
                <div class="noticias-pendientes" id="lista-noticias">
                    <?php foreach ($reportes as $reporte): 
                        $noticia_id = $reporte['propuestas_denuncias_id'];
                        ...
                        <div class="noticia-card">
                            ...
                            <div class="noticia-contenido">
                                ...
                                <div class="noticia-acciones">
                                    <form action="eliminar_reporte_denuncia.php" method="GET" style="flex: 1; margin-right: 10px;" onsubmit="return confirm('¿Deseas eliminar esta denuncia?')">
                                    ...
                                    <form method="GET" action="notificacion_reporte_denuncia.php">
                                        <input type="hidden" name="id" value="<?= htmlspecialchars($reporte['propuestas_denuncias_id']) ?>">
